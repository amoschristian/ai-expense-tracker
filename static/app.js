...
function MortgageView({ data }) {
    ...
    const startMonth = 12; // Dec 2023 = month 12
    ...
    function simulate(savingsBalance) {
        ...
        for (let m = 0; m < data.tenor; m++) {
            ...
            // Recalculate PMT only at rate change boundaries
    ...
    // Actual balance simulation with +Rp 2M/mo growth
    ...
    function simulateWithGrowth(startBalance) {
        ...
        for (let m = 0; m < data.tenor; m++) {
            ...
            // Grow savings (skip months before now)
            const elapsed = m;
    ...
    const actualNoSavings = simulateWithGrowth(data.house_balance);
    ...
    // Build trajectory chart data (group by rate period)
