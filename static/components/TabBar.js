...
const icons = {
    ...
    more: html`<svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><circle cx="4" cy="10" r="1.4" fill="currentColor"/><circle cx="10" cy="10" r="1.4" fill="currentColor"/><circle cx="16" cy="10" r="1.4" fill="currentColor"/></svg>`,
};

const labels = { summary: 'Summary', transactions: 'Transactions', recurring: 'Recurring', more: 'More' };

export function TabBar({ active, onChange, onAdd, onMore }) {
    const tabs = ['summary', 'transactions', 'recurring', 'more'];
    const items = [];
    tabs.forEach((t, i) => {
        items.push(html`
            <button
                ...
                onClick=${t === 'more' ? (onMore || (() => onChange(t))) : (() => onChange(t))}
            >
